...
import { getNavbarData, getPropertyData } from "@/actions/data-loaders";
...
const HotelSingleV1Dynamic = async ({ params }) => {
  ...
  const hotel = hotelsData.find((item) => item.slug == slug) || hotelsData[0];

  const navbarData = await getNavbarData();
  const propertyData = await getPropertyData(slug);
  console.dir(propertyData, { depth: null });
  const { title, location, travellers, ratings, review_count } = propertyData;
  ...
  return (
    <>
      ...
      <TopBreadCrumb title={title} location={location} />
      {/* End top breadcrumb */}

      <StickyHeader hotel={propertyData} />
      {/* sticky single header for hotel single */}

      <GalleryOne hotel={propertyData} />
        ...
        <div className="container">
          <div className="row y-gap-30">
            ...
            <div className="col-xl-4">
              <SidebarRight hotel={propertyData} />
            </div>
            {/* End .col-xl-4 */}
          </div>
          {/* End .row */}
